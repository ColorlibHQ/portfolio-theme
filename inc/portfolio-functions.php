...
function portfolio_opt( $id = null ){
...
// Rating function
function portfolio_rating_function( $rating = 5 ) {
	$html = '';
	for( $i = 0; $i < $rating; $i++ ) {
...
// Tags function
function portfolio_get_tax_function( $taxonomy = 'portfolio-tag' ) {
...
// Tags function
function portfolio_get_taxonomies( $taxonomy = 'listing_category', $hide_empty = false ) {
	$args = [
		'taxonomy' => $taxonomy,
...
function portfolio_meta( $id = '', $value_type = true ){
    
    $value = get_post_meta( get_the_ID(), '_portfolio_'.$id, $value_type );
    
    return $value;
}

// portfolio
function portfolio_get_styles_from_meta( $footer_bg_opt ) {
	$bg_styles = '';
...
// Blog Date Permalink
function portfolio_blog_date_permalink(){
...
if ( ! function_exists( 'portfolio_excerpt_length' ) ) {
	function portfolio_excerpt_length( $limit = 30 ) {
...
if ( ! function_exists( 'portfolio_posted_comments' ) ) :
    function portfolio_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','portfolio');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','portfolio');
            } else {
                $comments = esc_html__( '1 Comment','portfolio' );
            }
            $comments = '<a href="' . esc_url( get_comments_link() ) . '">'. $comments .'</a>';
        } else {
            $comments = esc_html__( 'Comments are closed', 'portfolio' );
        }
...
function portfolio_iframe_match(){   
    $audio_content = portfolio_embedded_media( array('audio', 'iframe') );
    $iframe_match = preg_match("/\iframe\b/i",$audio_content, $match);
    return $iframe_match;
}

//Post embedded media
function portfolio_embedded_media( $type = array() ){
...
function portfolio_social_sharing_buttons( $ulClass = '', $tagLine = '', $show_label = false ) {
...
if( ! function_exists('portfolio_blog_single_post_navigation') ) {
	function portfolio_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'portfolio_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'portfolio' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'portfolio' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'portfolio_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
// Portfolio Comment Template Callback
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'portfolio' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'portfolio'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'portfolio' ), '  ', '' ); ?> </p>
...
add_filter('comment_reply_link', 'portfolio_replace_reply_link_class');
function portfolio_replace_reply_link_class( $class ) {
...
// theme logo
function portfolio_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'portfolio_site_logo_83x39' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="'.esc_attr( portfolio_image_alt( $imageUrl[0] ) ).'"></a>';
	}else{
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( PORTFOLIO_DIR_IMGS_URI . 'logo.png' ).'" alt="'.esc_attr( get_bloginfo('name') ).'"></a>';
	}
...
function portfolio_theme_footer_logo( $class = '' ) {
	$siteUrl = home_url('/');
	$footer_logo_img_id = get_theme_mod( 'footer_logo_img' );
	$footer_logo_img 	= wp_get_attachment_image_src( $footer_logo_img_id , 'portfolio_site_footer_logo_101x60' );

	if( !empty( $footer_logo_img[0] ) ){
		$footer_logo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $footer_logo_img[0] ).'" alt="'.esc_attr( portfolio_image_alt( $footer_logo_img[0] ) ).'"></a>';
	}else{
		$footer_logo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( PORTFOLIO_DIR_IMGS_URI . 'footer_logo.png' ).'" alt="'.esc_attr( get_bloginfo('name') ).'"></a>';
	}
...
function portfolio_pull_right( $id = '', $condation ){
...
// image alt tag
function portfolio_image_alt( $url = '' ){
...
function portfolio_get_textareahtml_output( $content ) {
...
function portfolio_get_slider_shortcode_options( $field ) {
...
// html Style tag
function portfolio_inline_bg_img( $bgUrl ){
    $bg = '';
...
function portfolio_sidebar_opt(){

    $sidebarOpt = portfolio_opt( 'portfolio-blog-sidebar-settings' );
    $sidebar = '1';
...
function portfolio_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'portfolio' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'portfolio' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
// Themify Icon
function portfolio_themify_icon(){
    return [
        '3d-icon' 		=> __('3D Icon', 'portfolio'),
        'layers-icon' 	=> __('Layers Icon', 'portfolio'),
        'design-icon' 	=> __('Design Icon', 'portfolio'),
    ];
}

// CTA section
function portfolio_get_cta_area() {
	$cta_section_title      = !empty( portfolio_opt( 'portfolio_cta_section_title') ) ? portfolio_opt( 'portfolio_cta_section_title') : '';
	$cta_section_text       = !empty( portfolio_opt( 'portfolio_cta_section_text') ) ? portfolio_opt( 'portfolio_cta_section_text') : '';
	$cta_section_btn_label  = !empty( portfolio_opt( 'portfolio_cta_section_btn_label') ) ? portfolio_opt( 'portfolio_cta_section_btn_label') : '';
	$cta_section_btn_url    = !empty( portfolio_opt( 'portfolio_cta_section_btn_url') ) ? portfolio_opt( 'portfolio_cta_section_btn_url') : '';
	?>
    <!-- cta-area-start -->
...
function portfolio_get_contact_details_area() {
	?>
    <!-- location-area-start -->
    <div class="addres-area black-bg section-padding">
        <div class="container">
            <div class="row">
                <?php
				// Contact items
				$contact_items = portfolio_opt('portfolio_contact_details_section_fields');
                if ( $contact_items == true ) {
                    ...
                    if( is_array( $contact_items ) && count( $contact_items ) > 0 ){
                        foreach ($contact_items as $value) {
                            switch ($counter) {
                                case $counter == 2:
                                    $icon_img = PORTFOLIO_DIR_ICON_IMG_URI . 'clock.png';
                                    break;
                                case $counter == 3:
                                    $icon_img = PORTFOLIO_DIR_ICON_IMG_URI . 'envelope.png';
                                    break;
                                default:
                                    $icon_img = PORTFOLIO_DIR_ICON_IMG_URI . 'globe.png';
                                    break;
                            }
                            $counter++;
